...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const pbiWorkspacesTreeProvider = new PowerBIWorkspacesTreeProvider(context);
  ...
  const pbiCapacitiesTreeProvider = new PowerBICapacitiesTreeProvider(context);
  ...
  const pbiGatewaysTreeProvider = new PowerBIGatewaysTreeProvider(context);
  ...
  const pbiPipelinesTreeProvider = new PowerBIPipelinesTreeProvider(context);
  ...
  vscode.commands.registerCommand('PowerBI.initialize', async () => {
    const isValidated: boolean = await ThisExtension.initialize(context);
    if (!isValidated) {
  ...
  if (!ThisExtension.isInBrowser && PowerBIConfiguration.isTMDLEnabled) {
    ...
    if (vscode.workspace.workspaceFolders) {
      const firstTmdlFolder = vscode.workspace.workspaceFolders.find((folder) => folder.uri.scheme == TMDL_SCHEME);
